...
### Added
- Log separator written at the start of each CLI invocation now redacts secret option values (e.g. `--client-secret`) so credentials are never written to the log file in plain text.
- Authentication context (tenant, user, and subscription) is now logged at the `Information` level each time the cached authentication token changes, giving operators a clear audit trail in the log file without exposing credentials.
